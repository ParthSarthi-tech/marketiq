...
const STOCK_FUNDAMENTALS: Record<string, StockData> = {
  ...
  HDFC: {
    symbol: "HDFC",
    companyName: "Housing Development Finance Corp",
    currentSales: 85000,
    currentOPM: 35.2,
    currentPE: 20.0,
    currentEPS: 95.0,
...
export async function preloadAllStockData(): Promise<void> {
  ...
  const promises = tickers.map((ticker) => loadFromCSV(ticker));
